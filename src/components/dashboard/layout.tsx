...
import { News as SidebarNewsComponent } from "@/components/ui/sidebar-news";
...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  const user = { firstName: "User", lastName: "Name", emailAddresses: [{ emailAddress: "user@example.com" }] };
  const isLoaded = true;
  const isSignedIn = true;
  // const { signOut } = useClerk();
  const signOut = () => router.push('/authentication/login');
    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          name: "Agent Plan",
          icon: <Workflow size={18} />,
          href: "/tools/agent-plan",
          active: pathname.includes("/tools/agent-plan")
        },
        {
          name: "Talk to Xander",
  ...
  const SidebarContent = ({ mobile = false }: { mobile?: boolean }) => (
    <>
      ...
      <div className="mt-auto">
        ...
        {/* {false && <SidebarNewsComponent articles={[]} />} */}
  ...
  const employeeName = isSignedIn ? (`${user?.firstName} ${user?.lastName}` || "User") : "Guest User";
