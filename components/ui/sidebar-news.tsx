      ...
      <div className="relative w-full h-full">
        {cards.toReversed().map(({ href, title, summary, image }, idx) => (
          <div
            key={href}
            className={cn(
              "absolute left-0 top-0 w-full h-full scale-[var(--scale)] transition-[opacity,transform] duration-200",
              cardCount - idx > 3
                ? [
        ...
        {showCompleted && !cardCount && (
          <div
            className="animate-slide-up-fade absolute inset-0 flex w-full h-full flex-col items-center justify-center gap-3 [animation-duration:1s]"
    ...
    >
      <div className={hideContent ? "invisible" : ""}>
        <div className="flex flex-col gap-1">
          <span className="line-clamp-1 font-medium text-foreground text-xs">
            {title}
          </span>
          <p className="line-clamp-2 h-8 leading-4 text-[11px] text-muted-foreground">
            {description}
          </p>
        </div>
        <div className="relative mt-2 aspect-[16/9] w-full shrink-0 overflow-hidden rounded-sm border bg-muted">
          {image && (
            <Image
              src={image}
              alt=""
        ...
        <div
          className={cn(
            ...
            "sm:group-has-[*[data-dragging=true]]:h-6 sm:group-has-[*[data-dragging=true]]:opacity-100 sm:group-hover:group-data-[active=true]:h-6 sm:group-hover:group-data-[active=true]:opacity-100"
          )}
        >
          <div className="flex items-center justify-between pt-2 text-[10px]">
            <Link
              href={href || "https://xanderai.com"}
              target="_blank"
